...
#------------------------------------------------------------#
# Functions from here down are to be written by students
#------------------------------------------------------------#
...
# Testing
#split_points(nr$x[,1])
#LOF(y~.,data.frame(y=nr$y,x=as.numeric(nr$x[,1] <= 0)))
#split(nr,1,0) # calls split.region because nr is of class region
#rp <- recpart_recursive(nn)
#rp2 <- recpart(x,y)
#all.equal(rp,rp2)
#rpdebug <- recpart(x,y,debug=TRUE)
#---------------------------------------------------#
# Utility functions
#---------------------------------------------------#
